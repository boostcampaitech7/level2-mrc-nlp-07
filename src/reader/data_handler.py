...
class DataHandler:
    ...
    def load_data(self, type: str) -> dict:
        # type을 train 또는 validation을 지정해 출력
        ...
        self.question_column_name = 'question' if 'question' in column_names else column_names[
            0
        ]
        ...
        dataset = datasets[type].map(
            (
                self.prepare_train_features if type ==
                'train' else self.prepare_validation_features
            ),
            batched=True,
