...
from src.retriever.score.ranking import calculate_linear_score
...
class SparseRetrieval:
    ...
    def _initialize_from_wiki(self, context_path: str):
        with open(os.path.join(self.data_path, context_path), encoding='utf-8') as f:
            wiki = json.load(f)
    ...
    def retrieve(
        # Union은 주로 여러개의 typing 형이 가능할 때 주로 가용. Optinal은 1값이나 int형 값을 가질 수 있다는 의미.
        self, query_or_dataset: str | Dataset, topk: int | None = 1,
    ) -> tuple[list, list] | pd.DataFrame:  # 이 retrive의 결과로 튜플 형식의 (list, list)를 반환하거나 데이터프레임을 반환.
        """
        Arguments:
    ...
    def get_relevant_doc(self, query: str, k: int | None = 1) -> tuple[list, list]:
        """
        Arguments:
            query (str):
                하나의 Query를 받습니다.
        ...
        with timer('query ex search'):
            # (1, 50,000) x (50,000 x 문서 벡터 수) -> (1, 문서 수) -> 가장 유사한 문서를 찾을 수 있다..?
        ...
        # result를 1차원 벡터 -> 이를 위헤서 내림차순으로 정렬 -> 리스트로 변환 -> 상위 탑 k개만큼 slicing
