...
def reader():
    # CSV 파일에서 데이터 로드
    print("*" * 40,'Data Loading',"*" * 40)
    train_retriever_dataset = load_dataset('csv', data_files=str(SAVE_TRAIN_PATH))
    print(train_retriever_dataset)
    ...
    def process_answers(example):
        ...
        if isinstance(example[key_names.ANSWERS], str):
            example[key_names.ANSWERS] = ast.literal_eval(example[key_names.ANSWERS])
        return example

    # 'answers' 필드를 처리하여 파싱
    train_dataset_dict[key_names.TRAIN] = train_dataset_dict[key_names.TRAIN].map(process_answers)

    # Argument 설정
    print("*" * 40,'Checking ModelArguments',"*" * 40)
    model_args = ModelArguments()
    print("*" * 40,'Checking DataTrainingArguments',"*" * 40)
    data_args = DataTrainingArguments()
    print("*" * 40,'Checking TrainingArugments',"*" * 40)
